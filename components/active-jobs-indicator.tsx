    ...
    async function poll() {
      ...
      try {
        ...
        if (res.ok) {
          ...
          // Detect jobs that finished (were in previous list, now gone)
          const prevLabels = new Set(prevJobsRef.current.map((j) => j.label));
          const currentLabels = new Set(fetched.map((j) => j.label));
