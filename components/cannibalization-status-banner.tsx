...
import { Loader2, Split, XCircle, AlertTriangle } from "lucide-react";
...
export function CannibalizationStatusBanner({ run }: { run: Run | null }) {
  ...
  if (isRecentDone) {
    return (
      <Banner tone="success" icon={<Split className="h-4 w-4" strokeWidth={2} />}>
        <strong>Scan complete</strong> · {run.queriesScanned ?? 0} queries analysed ·{" "}
        {run.findingsCount ?? 0} finding{(run.findingsCount ?? 0) === 1 ? "" : "s"}
      </Banner>
    );
  }
  ...
  if (run.status === "skipped") {
    return (
      <Banner tone="warn" icon={<AlertTriangle className="h-4 w-4" strokeWidth={2} />}>
        <strong>Scan skipped</strong> — {run.error ?? "not ready"}
      </Banner>
    );
  }
