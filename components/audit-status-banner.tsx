...
export function AuditStatusBanner({ run }: { run: Run | null }) {
  ...
  if (run.status === "queued" || run.status === "running") {
    ...
    return (
      <Banner tone={tone} icon={icon}>
        <span>
          {isStale ? (
            <><strong>Audit stuck</strong> · {elapsed(run.startedAt ?? run.queuedAt, now)} — worker died</>
          ) : run.status === "running" ? (
            <>
            ...
            <><strong>Audit queued</strong> · {elapsed(run.queuedAt, now)}</>
          )}
        </span>
        <button
          onClick={onCancel}
  ...
  if (isRecentDone) {
    return (
      <Banner tone="success" icon={<Stethoscope className="h-4 w-4" strokeWidth={2} />}>
        <strong>Audit complete</strong> · {run.pagesCrawled ?? 0} pages, {run.findingsCount ?? 0}{" "}
        findings ({run.highSeverityCount ?? 0} high)
      </Banner>
    );
  }

  if (run.status === "failed") {
    const sqlDump = run.error?.startsWith("Failed query:");
    return (
  ...
  if (run.status === "skipped") {
    return (
      <Banner tone="warn" icon={<AlertTriangle className="h-4 w-4" strokeWidth={2} />}>
        <strong>Audit skipped</strong> — {run.error ?? "no site"}
      </Banner>
    );
  }
