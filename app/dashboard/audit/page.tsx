...
export default async function AuditPage() {
  ...
  const [latestRun] = await db
    ...
    .where(eq(schema.auditRuns.userId, session.user.id))
  ...
  return (
    <div className="px-8 lg:px-12 py-10 max-w-[1400px] mx-auto space-y-8">
      <header className="flex items-end justify-between gap-6 flex-wrap">
        <div>
          <p className="text-xs uppercase tracking-wider text-muted-foreground">Site audit</p>
          <h1 className="font-display text-5xl md:text-6xl mt-3">Audit</h1>
        </div>
        <RunAuditButton label={latestRun ? "Run new audit" : "Run first audit"} />
      </header>
      ...
      {synthesis && (
        <section className="rounded-[20px] bg-secondary p-6 md:p-8">
        ...
        <section>
          ...
          <div className="space-y-4">
            {Array.from(byUrl.entries()).map(([url, items]) => (
              <div key={url} className="rounded-[20px] bg-secondary overflow-hidden">
